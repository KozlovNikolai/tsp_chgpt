...
func (solver *TSPSolver) SolveFrom(start int) (int, []int) {
	if start < 0 || start >= solver.n {
		fmt.Println("Invalid start city")
		return -1, nil
	}

	solver.visited[start] = true
	solver.currentPath[0] = start
	for i := 0; i < solver.n; i++ {
		if i != start {
			solver.currentPath[i] = -1
		}
	}
	solver.solveRecursivelyFrom(start, 1, 0)
	return solver.minCost, solver.bestPath[:solver.n]
}

func (solver *TSPSolver) solveRecursivelyFrom(current, level, cost int) {
	if level == solver.n {
		if cost < solver.minCost {
			solver.minCost = cost
			copy(solver.bestPath, solver.currentPath)
		}
		return
	...
	for i := 0; i < solver.n; i++ {
		if !solver.visited[i] {
			solver.visited[i] = true
			solver.currentPath[level] = i
			solver.solveRecursivelyFrom(i, level+1, cost+solver.graph[current][i])
			solver.visited[i] = false
		}
	}
}
...
func main() {
	...
	// graph = replace(graph, 3-1, 11-1)
	// graph = replace(graph, 6-1, 10-1)
	// graph = replace(graph, 2-1, 4-1)
	// fmt.Println()
	// printMatrix(graph)
	fmt.Printf("Размер: %d\n", len(graph))
	solver := NewTSPSolver(graph)
	t := time.Now()
	minCost, bestPath := solver.SolveFrom(0)
	ts := time.Since(t)
	fmt.Printf("Time: %v\n", ts)
